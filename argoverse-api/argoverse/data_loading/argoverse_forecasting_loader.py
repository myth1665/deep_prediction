...
class ArgoverseForecastingLoader:
    ...
    def track_id_av(self) -> Sequence[int]:
        """Get the track ids in the current sequence.

        Returns:
            list of track ids in the current sequence
        """
        return np.unique(self.seq_df[self.seq_df["OBJECT_TYPE"]=="AV"]["TRACK_ID"].tolist())
    ...
    @property
    def av_traj(self) -> np.ndarray:
        """Get the trajectory for the track of type 'AGENT' in the current sequence.

        Returns:
            numpy array of shape (seq_len x 2) for the agent trajectory
        """
        av_x = self.seq_df[self.seq_df["OBJECT_TYPE"] == "AV"]["X"]
        av_y = self.seq_df[self.seq_df["OBJECT_TYPE"] == "AV"]["Y"]
        av_traj = np.column_stack((av_x, av_y))
        return av_traj
