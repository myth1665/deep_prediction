...
# print("test_size:", argoverse_test[0])
val_size = len(argoverse_val)
sample_size = len(argoverse_sample)
...
train_loader = DataLoader(argoverse_train, batch_size=64, shuffle=True)
test_loader = DataLoader(argoverse_test, batch_size=64, shuffle=True)
val_loader = DataLoader(argoverse_val, batch_size=32, shuffle=True)
sample_loader = DataLoader(argoverse_sample, batch_size=sample_size, shuffle=True)
...
# dataset_obsv = torch.stack((train_dataset['train_agent'],test_dataset['train_agent']))
# dataset_pred = torch.stack((train_dataset['gt_agent'], test_dataset['gt_agent']))
...
# Function to get batches  [[0,9],[10,19], ...]
...
the_batches = get_batches(len(dataset_obsv), batch_size)
...
def train():
    ...
    for ii, batch_i in enumerate(train_batches):
        ...
        if ii >= train_size - 1 or \
            ...
            print("the_batches[ii + 1][1] - the_batches[ii + 1][0]: ", the_batches[ii + 1][1] - the_batches[ii + 1][0])
            print("batch_size_accum:", batch_size_accum)
