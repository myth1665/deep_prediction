...
parser.add_argument('--epochs', '--e',
                    type=int, default=100000, metavar='N',
...
argoverse_test = Argoverse_Data('../deep_prediction/data/test_obs/data/')
...
trainData_size = len(argoverse_train)
testData_size = len(argoverse_test)
valData_size = len(argoverse_val)
sampleData_size = len(argoverse_sample)
...
# Define dataloaders 
print("Initiated Loaders . . . ")
train_loader = DataLoader(argoverse_train, batch_size=32, shuffle=True)
test_loader = DataLoader(argoverse_test, batch_size=32, shuffle=True)
val_loader = DataLoader(argoverse_val, batch_size=16, shuffle=True)
sample_loader = DataLoader(argoverse_sample, batch_size = sampleData_size, shuffle=True)

train_dataiter = iter(train_loader)
train_dataset = train_dataiter.next()

test_dataiter = iter(test_loader)
test_dataset = test_dataiter.next()

dataset_obsv = train_dataset['train_agent']
dataset_pred = train_dataset['gt_agent']
...
def train():
    ...
    # Get batch data
    train_dataiter = iter(train_loader)
    train_batch = train_dataiter.next()

    obsv = (train_batch['train_agent']).cuda()
    pred = (train_batch['gt_agent']).cuda()
...
#     print("obsv_4d type:", type(obsv_4d))

#         obsv_4d = torch.FloatTensor(obsv_4d)
#         pred_4d = torch.FloatTensor(pred_4d)
    ...
    ## Add to the summary writer
    writer.add_scalar('Loss/train', g_loss, epoch)
    writer.add_scalar('train_ADE', train_ADE, epoch)
    writer.add_scalar('train_FDE', train_FDE, epoch)
...
## Summary Writer
now = datetime.now()
log_dir = '../runs/' + now.strftime("%d-%m-%Y::%H:%M:%S") + '/'  
